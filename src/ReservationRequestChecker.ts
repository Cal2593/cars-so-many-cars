...
export function ReservationRequestChecker(
  ...
  switch (vehicleData[2]) {
    case 'Lorry':
      if (
        reservationRequest.elecRequired ||
        reservationRequest.covRequired ||
        reservationRequest.valRequired
      ) {
        responseNecessary = true;
        response.push(
          'Unfortunately we are unable to offer the following for lorries:'
        );
    ...
    case 'Motorhome/Caravan':
      if (
        ...
        if (!reservationRequest.elecRequired) {
          responseNecessary = true;
          response.push(
            'Would you like to add an electric hook up point to your booking?'
          );
          ...
          if (
            ...
            response.push(
              'We are unable to offer the following for Motorhomes and Caravans:'
            );
          }
        }
        const respLen: number = response.length;
        if (reservationRequest.covRequired) {
          responseNecessary = true;
          if (respLen > 0) {
            response.push(' Covered parking spaces;');
            ...
            response.push(
              'Unfortunately we are unable to offer the following for Motorhomes and Caravans:'
            );
            response.push(' Covered parking spaces;');
          }
        }
    ...
    case 'Car':
      if (
        (!reservationRequest.elecRequired && vehicleData[3] == true) ||
        !reservationRequest.covRequired ||
        !reservationRequest.valRequired ||
