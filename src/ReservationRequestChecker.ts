export function ReservationRequestChecker(reservationRequest: any,vehicleData:any){
let responseNecessary: boolean = false;
let response: string[] = [];
let FinalResponse: string | string[] = "";
    switch(vehicleData[2]){
        case "Lorry":
            if(reservationRequest.elecRequired || reservationRequest.covRequired || reservationRequest.valRequired){
        ...
        case "Motorhome/Caravan":
            if(!reservationRequest.elecRequired || reservationRequest.covRequired || reservationRequest.valRequired){
                if(!reservationRequest.elecRequired){
                    ...
                    if(reservationRequest.covRequired || reservationRequest. valRequired){
                        response.push("We are unable to offer the following for Motorhomes and Caravans:")
                    }
                }
                let respLen: number = response.length;
                if(reservationRequest.covRequired){
                    responseNecessary = true;
                    if(respLen > 0){
                        response.push(" Covered parking spaces;");
        ...
        case "Tricycle":
        case "Car":
            if((!reservationRequest.elecRequired && vehicleData[3] == true) || !reservationRequest.covRequired || !reservationRequest.valRequired){
                responseNecessary = true;
                ...
                let newResponse = responseChecker(response);
                FinalResponse = newResponse.slice(0,-1) + " to your booking?";
            }
        break;
    }
    if(responseNecessary) return FinalResponse;
    return responseNecessary;
}

function responseChecker(response: string[]){
    ...
    if(response.length == 2 || response.length == 3){
        ...
        if(response.length == 3){
            let midchunk = response[1].toString().slice(0,-1) + " or";
            response.splice(1,1,midchunk);
        }
    }
    if(response.length == 4){
        let chunkOne = response[1].toString().slice(0,-1) + ",";
        let chunkTwo = response[2].toString().slice(0,-1) + " or";
        let finalchunk  = response[3]
